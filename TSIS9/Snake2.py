...
class Snake:
    ...
    def add_to_snake(self):
        ...
        global L
        self.size += 1
        self.score += S #+1 to score if snace ate food
        self.elements.append([0, 0])
        self.is_add = False

        if self.score >= 10* L: #if snake ate more than 5 food its speed increases and it goes to the next level
            self.speed += 5
            self.level += 1
            L += 1
...
class Food:
    def draw(self, col):
        pygame.draw.rect(screen, col, (self.x, self.y, 10, 10))
...
while running:
    ...
    #generating position of food, so that it doesn't fall on a wall   
    if food.x == block.x + 20 or food.y == block.y + 20 or food.x == block.x - 20 or food.y == block.y - 20:
        INC_SPEED = pygame.USEREVENT + n
        pygame.time.set_timer(INC_SPEED, 15000)
        n+=1
        I =random.randint(1, 10)
